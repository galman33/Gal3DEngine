...
namespace Gal3DEngine
{
    public static class ShaderHelper
    {
        ...
        public static void TransformPosition(ref Vector4 position, Matrix4 transformation, Screen screen)
        {
            ...
            position.X = position.X / Math.Abs(position.W) * 0.5f * screen.Width;
            position.Y = position.Y / Math.Abs(position.W) * 0.5f * screen.Height;
            position.Z = position.Z / Math.Abs(position.W) * 0.5f;
        }
        ...
        {
            return (p1.Z < 0 || p1.Z > 1) && (p2.Z < 0 || p2.Z > 1) && (p3.Z < 0 || p3.Z > 1);
