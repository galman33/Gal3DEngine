...
namespace Gal3DEngine
{
    ...
    public class ShaderFlat : Shader<IndexPositionUVNormal, ShaderFlat.TriangleData, ShaderFlat.LineData>
    {
        ...
        protected override LineData MyProcessScanLine(float gradient1, float gradient2, IndexPositionUVNormal pa, IndexPositionUVNormal pb, IndexPositionUVNormal pc, IndexPositionUVNormal pd, TriangleData triData)
        {
            ...
            // perspective
            result.w1 = 1 / ShaderHelper.Lerp(1 / positions[pa.position].W, 1 / positions[pb.position].W, gradient1);
            result.w2 = 1 / ShaderHelper.Lerp(1 / positions[pc.position].W, 1 / positions[pd.position].W, gradient2);

            // starting Z & ending Z
            result.z1 = ShaderHelper.Lerp(positions[pa.position].Z / positions[pa.position].W, positions[pb.position].Z / positions[pb.position].W, gradient1) * result.w1;
            result.z2 = ShaderHelper.Lerp(positions[pc.position].Z / positions[pc.position].W, positions[pd.position].Z / positions[pd.position].W, gradient2) * result.w2;

            // starting uv & ending uv
            result.uv1 = ShaderHelper.Lerp(uvs[pa.uv] / positions[pa.position].W, uvs[pb.uv] / positions[pb.position].W, gradient1) * result.w1;
            result.uv2 = ShaderHelper.Lerp(uvs[pc.uv] / positions[pc.position].W, uvs[pd.uv] / positions[pd.position].W, gradient2) * result.w2;
        ...
        {
            var w = 1 / ShaderHelper.Lerp(1 / lineData.w1, 1 / lineData.w2, gradient);
            var z = ShaderHelper.Lerp(lineData.z1 / lineData.w1, lineData.z2 / lineData.w2, gradient) * w;
            Vector2 uv = ShaderHelper.Lerp(lineData.uv1 / lineData.w1, lineData.uv2 / lineData.w2, gradient) * w;
